...
let
  ...
  stripDevboxVersion =
    ...
    in
    if match == null then packageName else builtins.elemAt match 0;
  ...
  readDevboxPackages =
    ...
    in
    map stripDevboxVersion (parsed.packages or [ ]);
...
in
{
  ...
  fromDevboxJson =
    ...
    in
    wrapPreset presets.mkSandboxedClaudeCode (
      args
      // {
