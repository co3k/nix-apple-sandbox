...
手動で追加したい場合は `--sandbox-host-credential .codex/auth.json:.codex/auth.json` のような `source:target` 形式に加えて、`--sandbox-host-credential codex` / `gemini` / `claude` の alias も使える。通常は `nix-apple-sandbox codex` のようにコマンド名を渡せば `autoHostCredentialImportsByCommand` が効くため、手動指定は不要。
...
`hostCredentialImports` の要素は以下の形。`target` は `/home/sandbox` からの相対パスで、`..` や絶対パスは拒否される。`hostCredentialImportAliases` で runtime alias も追加できる。
